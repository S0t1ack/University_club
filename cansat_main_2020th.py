...
import kalman_integrated_program

gps = micropyGPS.MicropyGPS(9, 'dd') # MicroGPSオブジェクトを生成する。
                                     # 引数はタイムゾーンの時差と出力フォーマット
motor = motor.Motor(18, 25, 24,13, 17, 27)

#2021/03/02追加 カルマンフィルタでの角度取得
def run_get_angle():
    kalman_integrated_program.get_angle()
...
def init():
    ...
    # 2021/03/02追加
    anglethread = threading.Thread(target=run_get_angle, args=())
    anglethread.deamon = True
    anglethread.start()
...
#     center_y = get_cecter_y(maxY,minY)
    ...
    while True:
        if phase == 0:#x秒前進
            ...
            if myGps == None:
                return 0.0
            rover_and_goal_angle = math.atan2(target_x - myGps[0],target_y - myGps[1])
            e=[]
            e[0]=phi[0]+rover_and_goal_angle
            e.append(e[0])
            e.append(e[0])
            goal=0
            M=[100,0]
            ...
            while True:
                ...
                e[2] = e[1]
                e[1] = e[0]
                e[0] = goal - M[1] #偏差（e） = 目的値（goal） - 前回の操作量

                M[0] = M[1] + Kp * (e[0]-e[1]) + Ki * e[0] + Kd * ((e[0]-e[1]) - (e[1]-e[2]))

                motor.set_speed(f1(M[0]),f2(M[0]))
                time.sleep(1)
